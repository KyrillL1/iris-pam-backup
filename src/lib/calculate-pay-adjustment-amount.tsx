...
export function useCalculatePayAdjustmentAmount() {
    ...
    const calculatePayAdjustmentAmount = useCallback(async (payAdjustmentToEmployeesId: string, hoursWorked?: number): Promise<number> => {
        ...
        if (error || !data) {
            const message = `Missing response on calculatePayAdjustmentAmount for ${payAdjustmentToEmployeesId}`
        ...
        if (queryResult.length !== 1) {
            const message = `Invalid length for ${payAdjustmentToEmployeesId}: ${queryResult.length}`
            open?.({ message, type: "error" });
            console.error(message);
            return -1
        }
        ...
        const grossSalary = currentContract.calculation_basis === "HOURLY" ?
            ...
            currentContract.base_salary * currentContract.work_percentage / 100;
