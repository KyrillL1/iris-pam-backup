...
import { Autocomplete, Box, Button, Card, IconButton, MenuItem, TextField, Typography } from "@mui/material";
import { payAdjustmentOptionFactory } from "../pay-adjustment-option-factory";
import { useCallback, useEffect, useState } from "react";
import { Create as RefineCreate } from "@refinedev/mui";
...
import { EMPLOYEE_FIELD_NAME, mapIndexToCommentFieldName, mapIndexToCustomUserInputName, mapIndexToEndDateFieldName, mapIndexToFieldName, mapIndexToStartDateFieldName } from "./constants";
...
export default function PayAdjustmentsToEmployeesCreate() {
  ...
  const { payAdjustmentIds, payAdjustments, mapPayAdjustmentIdToName } = useFetchPayAdjustments();
  ...
  const [payAdjustmentFields, setPayAdjustmentFields] = useState<PayAdjustmentField[]>([]);
  ...
  return <RefineCreate
    title={<Typography component="h5" fontSize="1.5rem">Create Benefit / Deduction for Employee</Typography>}
    saveButtonProps={{
      ...saveButtonProps,
  ...
  >
    <Box component="form" sx={{
      ...
      {payAdjustmentFields.map(field => {
        const selectedPayAdjustmentId = watch(field.name) as string | undefined;
        const selectedPayAdjustment = payAdjustments.find(p => p.id === selectedPayAdjustmentId);

        return <Card key={field.key} sx={{ padding: 2, display: "flex", flexDirection: "column", gap: 2 }}>
          <Box sx={{ display: "flex", width: "100%", gap: 2 }}>
            ...
            />
            <IconButton color="error" onClick={() => handleRemovePayAdjustment(field)}><Remove /></IconButton>
          </Box>
          <Box sx={{ display: "flex", width: "100%", gap: 2, alignItems: "center", paddingLeft: 4 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: 'end' }}>
        <IconButton onClick={handleAddPayAdjustment} loading={!payAdjustments || payAdjustments.length === 0}><Add /></IconButton>
      </Box>
    </Box>
  </RefineCreate>;
}