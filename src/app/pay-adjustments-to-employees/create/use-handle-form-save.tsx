...
myI18n.addResourceBundle("en", "pay-adjustments-form", {
    unequalArrays: "Cannot save. Parsed arrays have unequal length",
    saveSuccess: "Adjustments saved successfully",
    supabaseError: "Supabase insert failed",
});

myI18n.addResourceBundle("pt", "pay-adjustments-form", {
    unequalArrays:
        "Não foi possível salvar. Arrays analisados têm comprimentos diferentes",
    saveSuccess: "Ajustes salvos com sucesso",
    supabaseError: "Falha ao inserir no Supabase",
});
...
export function useHandleFormSave<T extends FieldValues>() {
    ...
    const { t } = useTranslation("pay-adjustments-form");

    const handleFormSave = useCallback(
        async (data: FieldValue<T>) => {
            ...
            if (
                ...
                handleError(new Error(t("unequalArrays")), {
                    meta: {
                        parsedAdjustmentIds,
                        parsedUserInputs,
                        parsedComments,
            ...
            if (error) {
                handleError(new Error(t("supabaseError"), error));
            } else {
                open?.({
